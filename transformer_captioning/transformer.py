...
class AttentionLayer(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        dot_product = query @ key.transpose(-2,-1) / math.sqrt(self.embed_dim)
...
class MultiHeadAttentionLayer(AttentionLayer):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        dot_product = query.transpose(1,2) @ key.permute(0,2,3,1) / math.sqrt(self.embed_dim//H)
...
class SelfAttentionBlock(nn.Module):
    ...
    def forward(self, seq, mask):
        ...
        out_self_attn = self.self_attn(seq, seq, seq, mask) #TODO KRITI  - check if K, Q, V are correct 
        out = self.dropout(out_self_attn) + seq
...
class CrossAttentionBlock(nn.Module):
    ...
    def forward(self, seq, cond):
        ...
        # Then add a residual connection to the original input, and finally apply normalization. #############################
        out_cross_attn = self.cross_attn(seq, cond, cond) #TODO KRITI  - check if K, Q, V are correct
        out = self.dropout(out_cross_attn) + seq
        out = self.norm(out)
        return out
